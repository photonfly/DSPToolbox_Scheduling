...
for out_id = outputid
    schedule = changestarttime(schedule, 'out', out_id, Diff_time_output+offset+out_id*L);
end
printstarttimes(schedule);
%% To move output butterflies 
% From last layer to first layer
for layers = size(butterfly_mat,2) : -1 : 1 
    butteflyLayer = butterfly_mat(:,layers);
    %For each butterfly
    for i = 1: N/2
        btf_id_move = butteflyLayer(i); % The butterfly to be moved
        op_id = find_op_id(butterfly_array,N,btf_id_move);
        upperBound = getupperbound(schedule,op_id); 
        startTime = getstarttimes(schedule,'butterfly2',btf_id_move);
        distance = upperBound - startTime; % How much to move?
        schedule = changestarttime(schedule, 'butterfly2', btf_id_move, distance);
    end
end
schedule = changeendTime(schedule,320); % Change end time
...
% %     [anySpace, btf_id_move,min_value] = isAnySpace(schedule,outputid+1);
% % end
% % schedule = changeendTime(schedule,out_time+2); % Change end time
% printstarttimes(schedule);
% plotschedule(schedule);
% 
% %% Latencies
